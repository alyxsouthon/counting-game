...
public class CountingGame {
    ...
    //constructor
    public CountingGame(GameConfig config){
        this.config = config;
    }
    ...
    public void runGame() {
        ...
        int noOfAttempts = config.getMax_attempts();
        ...
        System.out.println("A number between " + config.getMin() + " and " + config.getMax() + " has been chosen.");
        ...
        while (playerWinCount < 2 && computerWinCount < 2) {
            for (int i = 0; i < noOfRounds; i++) {
                ...
                int number = random.nextInt(config.getMax()) + 1;
        ...
        if (playerWinCount >= 2) {
            System.out.println("You won the Best of 3. Congrats");
        } else {
